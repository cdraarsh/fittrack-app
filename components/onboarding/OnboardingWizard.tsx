...
export default function OnboardingWizard() {
  ...
  return (
    <div className="min-h-screen bg-paper flex items-center justify-center p-5">
      <div className="w-full max-w-[420px] bg-surface border border-hairline rounded-[20px] p-7 shadow-2xl">

        {/* Progress dots */}
        <div className="flex gap-1.5 mb-6">
          {Array.from({ length: TOTAL_STEPS }, (_, i) => i + 1).map(n => (
            <div key={n} className={`h-1 rounded-full flex-1 transition-all ${
              n < step ? 'bg-clay-dim' : n === step ? 'bg-clay' : 'bg-surface-2 border border-hairline'
            }`} />
          ))}
        </div>
        ...
        {step === 1 && (
          <>
            <div className="font-sans text-3xl font-black mb-1">Welcome to FitTrack</div>
            <div className="text-sm text-ink-3 mb-6">Quick setup — takes 2 minutes.</div>
        ...
        {step === 2 && (
          <>
            <div className="font-sans text-2xl font-black mb-1">Your Goal</div>
            <div className="text-sm text-ink-3 mb-5">This shapes your entire plan.</div>

            <Field label="Age" value={age} onChange={setAge} placeholder="e.g. 24" type="number" />

            <div className="mb-4">
              <div className="text-[10px] text-ink-3 uppercase font-bold mb-1.5 tracking-wider">Gender</div>
              <div className="flex gap-1.5">
                {(['male', 'female', 'other'] as const).map(g => (
                  <button key={g} onClick={() => setGender(g)}
                    className={`flex-1 py-2.5 rounded-sm text-sm font-bold border capitalize transition-all ${
                      gender === g ? 'bg-clay-wash border-clay-dim text-clay' : 'bg-surface-2 border-hairline text-ink-3'
                    }`}>
                    {g}
                  </button>
                ))}
            ...
            <div className="mb-4">
              <div className="text-[10px] text-ink-3 uppercase font-bold mb-1.5 tracking-wider">Primary Goal</div>
              <div className="flex flex-col gap-1.5">
                {([
                  ...
                  <button key={val} onClick={() => setGoal(val)}
                    className={`w-full py-3 px-4 rounded-sm text-left border transition-all ${
                      goal === val ? 'bg-clay-wash border-clay-dim' : 'bg-surface-2 border-hairline'
                    }`}>
                    <div className={`text-sm font-bold ${goal === val ? 'text-clay' : 'text-ink-2'}`}>{label}</div>
        ...
        {step === 3 && (
          <>
            <div className="font-sans text-2xl font-black mb-1">Your Lifestyle</div>
            <div className="text-sm text-ink-3 mb-5">Used to calculate your exact calorie needs.</div>

            <div className="mb-4">
              <div className="text-[10px] text-ink-3 uppercase font-bold mb-1.5 tracking-wider">Activity outside gym</div>
              <div className="flex flex-col gap-1.5">
                {([
                  ...
                  <button key={val} onClick={() => setActivity(val)}
                    className={`w-full py-2.5 px-4 rounded-sm text-left border transition-all ${
                      activity === val ? 'bg-clay-wash border-clay-dim' : 'bg-surface-2 border-hairline'
                    }`}>
                    <span className={`text-sm font-bold ${activity === val ? 'text-clay' : 'text-ink-2'}`}>{label}</span>
                    <span className="text-[10px] text-ink-3 ml-2">{desc}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="mb-4">
              <div className="text-[10px] text-ink-3 uppercase font-bold mb-1.5 tracking-wider">Training Experience</div>
              <div className="flex gap-1.5">
                {([
                  ...
                  <button key={val} onClick={() => setExperience(val)}
                    className={`flex-1 py-2.5 rounded-sm text-center border transition-all ${
                      experience === val ? 'bg-clay-wash border-clay-dim' : 'bg-surface-2 border-hairline'
                    }`}>
                    <div className={`text-xs font-bold ${experience === val ? 'text-clay' : 'text-ink-2'}`}>{label}</div>
                    <div className="text-[9px] text-ink-3 mt-0.5">{sub}</div>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="mb-4">
              <div className="text-[10px] text-ink-3 uppercase font-bold mb-1.5 tracking-wider">Diet Style</div>
              <div className="flex flex-wrap gap-1.5">
                {([
                  ...
                  <button key={val} onClick={() => setDietStyle(val)}
                    className={`px-3 py-2 rounded-sm text-xs font-bold border transition-all ${
                      dietStyle === val ? 'bg-clay-wash border-clay-dim text-clay' : 'bg-surface-2 border-hairline text-ink-3'
                    }`}>
                    {label}
                  </button>
                ))}
            ...
            <div className="mb-4">
              <div className="text-[10px] text-ink-3 uppercase font-bold mb-1.5 tracking-wider">Equipment Available</div>
              <div className="flex flex-col gap-1.5">
                {([
                  ...
                  <button key={val} onClick={() => setEquipment(val)}
                    className={`w-full py-2.5 px-4 rounded-sm text-left border transition-all ${
                      equipment === val ? 'bg-clay-wash border-clay-dim' : 'bg-surface-2 border-hairline'
                    }`}>
                    <span className={`text-sm font-bold ${equipment === val ? 'text-clay' : 'text-ink-2'}`}>{label}</span>
                    <span className="text-[10px] text-ink-3 ml-2">{desc}</span>
                  </button>
                ))}
              </div>
        ...
        {step === 4 && (
          <>
            <div className="font-sans text-2xl font-black mb-1">Program Schedule</div>
            <div className="text-sm text-ink-3 mb-6">When does your program start?</div>
            <Field label="Start date" value={start} onChange={setStart} type="date" />
            <div className="text-xs text-ink-3 -mt-2 mb-5">Set today if you're just starting. Set a past date to resume mid-program.</div>
            <div className="mb-5">
              <div className="text-[10px] text-ink-3 uppercase font-bold mb-2 tracking-wider">Program Length</div>
              <div className="flex gap-1.5">
                {[8, 10, 12, 16, 20].map(w => (
                  <button key={w} type="button" onClick={() => setProgramWeeks(w)}
                    className={`flex-1 py-2.5 rounded-sm text-sm font-bold border transition-all ${
        ...
        {step === 5 && (
          <>
            <div className="font-sans text-2xl font-black mb-1">Gym Days</div>
            <div className="text-sm text-ink-3 mb-5">Which days do you train?</div>
            ...
            <div className="bg-clay-wash border border-clay-dim rounded-sm p-3.5 mb-5">
              <div className="text-xs font-bold text-clay mb-1">AI Plan Generation</div>
            ...
            {!aiError ? (
              <>
                <div className="flex flex-col items-center py-6 text-center">
                  <div className="w-14 h-14 rounded-full bg-clay-wash border border-clay-dim flex items-center justify-center mb-5">
                    <Spinner />
                  </div>
                  <div className="font-sans text-2xl font-black mb-2">Building Your Plan</div>
                  <div className="text-sm text-ink-3 mb-4">Claude is analysing your profile...</div>
                          ...
                          return (
                            <div key={key} className="flex items-center gap-2 text-[11px]">
                              <span className={`w-4 h-4 rounded-full flex items-center justify-center text-[9px] font-black ${
                                done ? 'bg-clay-wash text-clay border border-clay-dim' : 'bg-surface-2 text-ink-3/40 border border-hairline'
                              }`}>
                                {done ? '✓' : '·'}
                              </span>
                              <span className={done ? 'text-text' : 'text-ink-3/60'}>{label}</span>
                            </div>
                          );
                        })}
                      </div>
                  ...
                  {aiPartial?.daily_targets?.gym_day_calories ? (
                    <div className="text-[11px] text-ink-3 mt-4">
              ...
              <>
                <div className="font-sans text-2xl font-black mb-2 text-red-400">Generation Failed</div>
                <div className="text-sm text-ink-3 mb-4 bg-surface-2 border border-hairline rounded-sm p-3 text-[11px] leading-relaxed break-words">
                  {aiError}
                </div>
                {tdee && (
                  <>
                    ...
                    <div className="mb-4">
                      <div className="text-[10px] text-ink-3 uppercase font-bold mb-2 tracking-wider">Calorie Targets</div>
                      <div className="flex gap-2 mb-3">
                        {(['auto', 'manual'] as Mode[]).map(m => (
                          <button key={m} onClick={() => setMode(m)}
                            className={`flex-1 py-2 rounded-sm text-sm font-bold border transition-all ${
                              mode === m ? 'bg-clay-wash border-clay-dim text-clay' : 'bg-surface-2 border-hairline text-ink-3'
                            }`}>
                      ...
                      {mode === 'auto' ? (
                        <div className="bg-surface-2 border border-hairline rounded-sm p-4 grid grid-cols-2 gap-2">
                          {([['Gym day', `${tdee.cals_gym} kcal`], ['Rest day', `${tdee.cals_rest} kcal`], ['Protein', `${tdee.protein}g`], ['Fat', `${tdee.fat}g`]] as const).map(([l, v]) => (
                            <div key={l} className="bg-surface-2 rounded-lg p-2.5 text-center">
                              <div className="font-sans text-xl font-black text-clay">{v}</div>
                              <div className="text-[10px] text-ink-3 uppercase font-bold mt-0.5">{l}</div>
                            </div>
                          ))}
                        </div>
                      ) : (
...
function Field({ label, value, onChange, placeholder, type = 'text' }: {
  ...
  return (
    <div className="mb-4">
      <div className="text-[10px] text-ink-3 uppercase font-bold mb-1.5 tracking-wider">{label}</div>
      <input
...
function Btn({ children, onClick, secondary }: { children: React.ReactNode; onClick: () => void; secondary?: boolean }) {
  return (
    <button onClick={onClick} className={`flex-1 py-3.5 rounded-sm font-bold text-sm transition-all ${
      secondary
        ? 'bg-surface-2 border border-hairline text-ink-2 flex-none px-5'
        : 'bg-clay hover:bg-clay-hover text-surface'
    }`}>
      {children}
    </button>
  );
...
function Spinner() {
  return (
    <svg className="w-6 h-6 animate-spin text-clay" viewBox="0 0 24 24" fill="none">
